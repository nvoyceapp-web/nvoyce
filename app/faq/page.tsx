  ...
  },
  {
    ...
    faqs: [
      ...
      },
      {
        ...
        answer:
          "Every invoice has a Stripe payment link and QR code generated when you send it. To share either, open the Actions dropdown on the invoice row:\n\n• Copy Payment Link — copies the Stripe checkout URL to your clipboard so you can paste it via text, WhatsApp, or any other channel.\n\n• Show QR Code — opens a QR code modal your client can scan with their phone camera to go straight to checkout. You can also download the QR code as a PNG.\n\nBoth options only appear for invoices that have a Stripe link attached and are not in Draft status.",
      },
      {
        id: 'notifications',
          ...
          {faqGroups.map((group) => (
            <div key={group.label}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
                <span style={{ color: 'var(--orange)', display: 'flex', alignItems: 'center' }}>{group.icon}</span>
                  ...
                  <div key={faq.id} style={{ background: 'white', borderRadius: 12, border: `1px solid ${expandedId === faq.id ? 'var(--orange)' : 'var(--line)'}`, overflow: 'hidden', transition: 'border-color 200ms' }}>
                    ...
                    >
                      ...
                      <svg
                        ...
                        style={{ flexShrink: 0, transition: 'transform 200ms', transform: expandedId === faq.id ? 'rotate(180deg)' : 'rotate(0deg)' }}
                      >
